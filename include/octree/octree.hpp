...
namespace gutil
{
	...
	struct Octree
	{
		...
		InsertReturn insert(value_type const& value);
		InsertReturn insert(value_type&& value);
		void insert(std::span<const value_type> values);
		void insert(std::span<value_type> values);
	...
	protected:
		...
		NodeIndex _root_index {0, NodeTag::LEAF};
	...
	void Octree<O,V,D>::swap_internal(const size_type idx_A, const size_type idx_B) {
		...
		if (is_root(NodeIndex::internal(idx_A))) {_root_index_.index = idx_B;}
		else if (is_root(NodeIndex::internal(idx_B))) {_root_index_.index = idx_A;}
	...
	template<NodeOpts O, typename V, typename D>
	NodeIndex Octree<O,V,D>::find_branch(const value_type& value) {
		if constexpr (O::SINGLE_DATA) {
			...
			while (idx.tag != NodeIndex::LEAF) {
				...
				if (n_valid_children > 1) {return last_valid;}
