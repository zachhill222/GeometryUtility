...
namespace gutil
{
	...
	struct NodeIndex
	{
		...
		constexpr NodeIndex(const size_t idx, const NodeTag t) : index(idx), tag(t) {}

		//comparisons
		bool operator==(const NodeIndex& other) const {return (index==other.index) && (tag==other.tag);}
		bool operator!=(const NodeIndex& other) const {return (index!=other.index) || (tag!=other.tag);}
		...
		static constexpr NodeIndex null() {return {NULL_NODE, NodeTag::INTERNAL};}
	};
	...
	struct LeafNode
	{
		...
		//iterators for easy looping over data
	...
	struct InternalNode
	{
		...
		//iterators for easy looping over children indices
